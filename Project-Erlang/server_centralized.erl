...
follow(Users, UserName, UserNameToFollow) ->
    {user, Name, Subscriptions, Followers, Messages} = get_user(UserName, Users),
    NewUser = {user, Name, sets:add_element(UserNameToFollow, Subscriptions), Followers, Messages},
...
store_message(Users, Message) ->
    ...
    {user, Name, Subscriptions, Followers, Messages} = get_user(UserName, Users),
    NewUser = {user, Name, Subscriptions, Followers, Messages ++ [Message]},
    dict:store(UserName, NewUser, Users).

% Get all messages by `UserName`.
get_messages(Users, UserName) ->
    {user, _, _, _, Messages} = get_user(UserName, Users),
    Messages.
